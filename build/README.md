...
4. Build Simstrat with the `release-gnu-aed-full` FoBiS mode instead of `release-gnu`: `FoBiS.py build -mode release-gnu-aed-full`. This mode additionally links `libaed-riparian`, `libaed-lighting`, `libaed-dev` and `libPhreeqcRM`, plus `-fopenmp -lstdc++` for phreeqcrm's C++/OpenMP dependencies. On macOS, use `release-gnu-aed-full-dynamic` instead (mirrors how `release-gnu-dynamic` relates to `release-gnu`) — macOS's linker doesn't support fully static binaries, so the `-static` variant will fail or crash there.
